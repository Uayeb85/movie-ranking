<template>
  <header class="bg-white sm:flex sm:justify-end sm:items-end">
    <div class="flex items-center justify-end px-4 py-4 ">
      <div class="sm:hidden">
        <button @click="isOpen = !isOpen"  type="button" class=" block text-black focus:text-black focus:outline-none hover:text-black">
      ...
      <div :class="isOpen ? 'block' : 'hidden'" class="px-4 pt-2 pb-4 sm:flex"  >
        <g-link to="/" class="mt-1 block px-2 py-1 text-black font-semibold rounded hover:text-orange-400 sm:mt-0 sm:ml-2">Inicio</g-link>      
        <g-link to="/movies/" class="mt-1 block px-2 py-1 text-black font-semibold rounded hover:text-orange-400 sm:mt-0 sm:ml-2" >Peliculas</g-link>
      </div> 
  </header>
</template>
